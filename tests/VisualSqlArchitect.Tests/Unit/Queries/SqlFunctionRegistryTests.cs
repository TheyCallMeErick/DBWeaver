...
public class SqlFunctionRegistryTests
{
    ...
    public void Regex_Postgres_UsesNativeTildeOperator()
    {
        ...
        var sql = reg.GetFunction(SqlFn.Regex, "email", "'@gmail\\.com'");
    ...
    public void Regex_SqlServer_UsesPATINDEX()
    {
        ...
        var sql = reg.GetFunction(SqlFn.Regex, "email", "'%@gmail%'");
    ...
    {
        var reg = new SqlFunctionRegistry(DatabaseProvider.Postgres);
        var sql = reg.GetFunction(SqlFn.DateDiff, "created_at", "NOW()");
    ...
    {
        var reg = new SqlFunctionRegistry(DatabaseProvider.SqlServer);
        var sql = reg.GetFunction(SqlFn.DateDiff, "created_at", "GETDATE()");
    ...
    {
        var reg = new SqlFunctionRegistry(DatabaseProvider.MySql);
        var sql = reg.GetFunction(SqlFn.StringAgg, "name", "','");
    ...
    {
        foreach (var p in new[] { DatabaseProvider.SqlServer, DatabaseProvider.Postgres })
        {
    ...
    {
        var reg = new SqlFunctionRegistry(DatabaseProvider.SqlServer);
        var sql = reg.GetFunction(SqlFn.Greatest, "a", "b", "c");
    ...
    public void GetFunction_UnknownName_ThrowsNotSupported()
    {
        ...
        Assert.Throws<NotSupportedException>(
            () => reg.GetFunction("FAKE_FUNCTION", "col"));
    }
}
...
public class QueryBuilderServiceTests
{
    ...
    {
        var svc = QueryBuilderService.Create(p);
        var result = svc.Compile(new VisualQuerySpec("users"));
    ...
    {
        var svc = QueryBuilderService.Create(DatabaseProvider.SqlServer);
        var result = svc.Compile(new VisualQuerySpec(
            "users",
            Selects: new[] { new SelectColumn("name", "UserName") }
        ));
    ...
    {
        var svc = QueryBuilderService.Create(DatabaseProvider.Postgres);
        var result = svc.Compile(new VisualQuerySpec(
            "users",
            Selects: new[] { new SelectColumn("name", "UserName") }
        ));
    ...
    public void Compile_RegexFilter_Postgres_InjectsNativeOperator()
    {
        ...
        var result = svc.Compile(new VisualQuerySpec(
            ...
            Filters: new[]
            {
                new FilterDefinition("email", "REGEX", "'@corp\\.io'")
            }
        ));
    ...
    public void Compile_RegexFilter_SqlServer_UsesPATINDEX()
    {
        ...
        var result = svc.Compile(new VisualQuerySpec(
            ...
            Filters: new[]
            {
                new FilterDefinition("email", "REGEX", "'%@corp%'")
            }
        ));
    ...
    {
        var svc = QueryBuilderService.Create(DatabaseProvider.Postgres);
        var result = svc.Compile(new VisualQuerySpec("orders", Limit: 50, Offset: 100));
    ...
    public void Compile_Pagination_SqlServer_UsesOffsetFetch()
    {
        ...
        var result = svc.Compile(new VisualQuerySpec(
            "orders",
            Orders: new[] { new OrderDefinition("id") },
            Limit: 50,
            Offset: 100
        ));
...
public class ConnectionConfigTests
{
    ...
    public void BuildConnectionString_MySQL_ContainsServer()
    {
        var cfg = new ConnectionConfig(
            ...
            "mysqlhost", 3306, "mydb", "root", "secret");
        var cs = cfg.BuildConnectionString();
