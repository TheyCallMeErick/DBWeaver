...
public enum GuardSeverity { Warning, Block }
...
public static class QueryGuardrails
{
    ...
    public static IReadOnlyList<GuardIssue> Check(string sql)
    {
        ...
        bool isSelect = normalised.StartsWith("SELECT", StringComparison.Ordinal)
    ...
    {
        bool hasLimit = sql.Contains(" LIMIT ")   || sql.Contains("\nLIMIT ")
                     || sql.Contains(" LIMIT\n")
                     || sql.Contains(" TOP ")
                     || sql.Contains("FETCH NEXT") || sql.Contains("FETCH FIRST")
                     || sql.Contains("ROWNUM ");

        if (!hasLimit)
            issues.Add(new GuardIssue(
    ...
    private static void CheckSelectStar(string sql, List<GuardIssue> issues)
    {
        ...
        bool hasStar = sql.Contains("SELECT *")
    ...
    {
        // Skip if query is a simple schema inspection or has aggregation
        ...
        bool hasJoin    = sql.Contains(" JOIN ");

        // Only flag plain FROM-without-filter scenarios
        if (!hasWhere && !hasHaving && !hasGroupBy && sql.Contains(" FROM "))
            issues.Add(new GuardIssue(
                GuardSeverity.Warning,
                "NO_FILTER",
                "Query has no WHERE or HAVING clause — may perform a full table scan",
                "Add a WHERE clause to filter rows and improve performance"));
    }
}
